C   ******************** MultPlane3 ************************
C
C   CALLING PROGRAM FOR E-FIELD AND TUNNEL CURRENT COMPUTATIONS FOR 
C   TIP NEAR UNIFORM SEMICONDUCTOR, IN 3D. CURRENT COMPUTED USING 
C   A PLANE WAVE EXPANSION.
C
C   VERSION 6.2 - NOV/12, CONSTRUCTED FROM UniPlane3_6.2 AND MultInt3_6.2
C           6.3 - OCT/15, INCLUDE OUTPUT OF SURFACE CHARGE DENSITIES; PASS NE TO ENFIND
C
...
C
C   CALLS SURFRHOMULT VERSION 6.2 FOR EVALUATING SURFACE CHARGE DENSITIES.
C
C   CALLS POTCUT3 VERSION 6.0 FOR OBTAINING POTENTIAL PROFILES.
C
C   CALLS PLANECURR3 VERSION 6.0 FOR SOLVING SCHRODINGER'S EQN.
C
...
C
     ...
     &NVDIM1=NVDIM+1,NVDIM2=2048,NSDIM2=20000,NEDIM=50000,NXDIM=64,
     &NXDIM2=2*NXDIM-2,NYDIM=64,NZDIM=64,NEIGENDIM=8200,NREGDIM=2,
     &NARDIM=2)
C      
      DIMENSION VAC(2,NRDIM,NVDIM,NPDIM),SEM(2,NRDIM,NSDIM,NPDIM),
     &VSINT(2,NRDIM,NPDIM),R(NRDIM),S(NSDIM),DELV(NRDIM),ITMAX(10),
     &EP(10),BBIAS(1000),BARR(NVDIM1),PROF(NSDIM),EMAX(4),
     &PVAC(NXDIM2,NYDIM,NVDIM),PSEM(NXDIM2,NYDIM,NZDIM),
     &PSURF(NXDIM2,NYDIM),BARR2(NVDIM2),AVBL(NREGDIM),
...
C
      READ(9,*) MIRROR
      IF (MIRROR.EQ.1) WRITE(6,*) 'HORIZONTAL MIRROR PLANE ASSUMED'
      IF (MIRROR.EQ.1) WRITE(16,*) 'HORIZONTAL MIRROR PLANE ASSUMED'
      READ(9,*) NRIN
      READ(9,*) NVIN
      READ(9,*) NSIN
      READ(9,*) NPIN
...
C
     ...
     &   PotTIP,IWRIT1,ITMAX,EP,IPMAX,Pot0,IERR,IINIT,MIRROR,EPSIL)
      WRITE(6,120) NR,NS,NV,IERR
      WRITE(16,120) NR,NS,NV,IERR
120   FORMAT(' RETURN FROM SEMTIP2, NR,NS,NV,IERR =',4I7)
      WRITE(10,130) RAD,SEP,BIAS,CPot,Pot0
130   FORMAT(' ',5G12.4)
C
C   GET POTENTIAL PROFILE
C
      CALL POTCUT3(0,VAC,TIP,SEM,VSINT,NRDIM,NVDIM,NSDIM,NPDIM,
     &   NV,NS,NP,SEP,S,DELV,Pot0,BIAS,CHI,CPot,EGAP,BARR,PROF,NBARR1,
     &   NVDIM1,NVDIM2,IWRIT)
C     CALL POTCUT1(0,VAC,TIP,SEM,VSINT,NRDIM,NVDIM,NSDIM,
C     &NV,NS,SEP,S,R,DELV,PHI0,BIAS,CHI,DELPHI,BARR,PROF,NBARR1,
C     &NVDIM1,NVDIM2,PMIN,PMAX,0)
      VACSTEP=0.01
...
C
      IF (MIRROR.NE.1) THEN
         WRITE(6,*) '*** SOLUTION FOR CURRENT CAN ONLY BE DONE WITH '
     &      ,'MIRROR PLANE'
         WRITE(16,*) '*** SOLUTION FOR CURRENT CAN ONLY BE DONE WITH '
     &      ,'MIRROR PLANE'
         GO TO 500
      END IF
      ICOMP=1
      IF (ICOMPIN.EQ.0) THEN
         IF (BIAS.GE.0.) THEN
            ICOMP=0
         ELSE
            ICOMP=-1
      ...
      ACB1=ACB(1)
      ESO1=ESO(1)
      CALL planecurr(SEP,VAC,TIP,SEM,VSINT,R,S,DELV,DELR,DELS,DELP,
     &NRDIM,NVDIM,NSDIM,NPDIM,NR,NV,NS,NP,NXDIM,NXDIM2,NYDIM,NZDIM,
     &PVAC,PSEM,PSURF,VACWID,CHI,EFTIP,EGAP1,AVBL1,AVBH1,AVBSO1,ACB1,
     &ESO1,BIAS,DELPHI,PHI0,TK,EF,EMAX,CURR,CURRV,CURRC,IWRIT,IERR,
     &NBARR2,NVDIM2,BARR2,NEIGENDIM,ZVACDEL,ICOMP)
C
...
C
      if (imod.eq.-1) then
         ...
         write(15,*) bias0,COND
         write(93,*) bias0,CONDV
         write(94,*) bias0,CONDC
      end if
...
500   IF (IWRIT.GE.1) THEN
         DO 505 J=NV,1,-1
           WRITE(11,*) -J*DELV(1),VAC(1,1,J,1)
505      CONTINUE
         WRITE(11,*) 0.,VSINT(1,1,1)
         DO 510 J=1,NS
            WRITE(11,*) S(J),SEM(1,1,J,1)
510      CONTINUE
C
...
C
         ...
         DO 515 I=NR,1,-1
            WRITE(12,*) -R(I),VSINT(1,I,KPLOT2)
515      CONTINUE
         DO 520 I=1,NR
            WRITE(12,*) R(I),VSINT(1,I,KPLOT1)
520      CONTINUE
         CLOSE(11)
         CLOSE(12)
      END IF
...
C
      ...
      END
C
C   ROUTINE DEFINING CHANGE IN VB EDGE
C
      REAL FUNCTION DELVBEDGE(X,Y,Z,I,J,K)
...
C
      ...
      END
C
C   ROUTINE DEFINING CHANGE IN CB EDGE
C
      REAL FUNCTION DELCBEDGE(X,Y,Z,I,J,K)
      COMMON/SEMI/TK,EGAP,ED,EA,ACBAV,AVB,CD,CA,IDEG,IINV
C
      DELCBEDGE=0.
      RETURN
      END
