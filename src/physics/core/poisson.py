...
def solve_poisson_hyperbolic(
    ...
    """Solve the Poisson equation on a prolate spheroidal grid using SOR.

    Parameters
    ----------
    eta_grid, nu_grid : np.ndarray
        Coordinate grids generated by :func:`generate_prolate_spheroidal_grid`.
    phi_tip : float
        Potential on the tip surface (\u03b7 constant boundary).
    phi_sample : float
        Potential on the sample surface (\u03bd = 0 line).
    ...
    charge_density : np.ndarray, optional
        Charge density array with same shape as ``eta_grid``.

    Returns
    -------
    np.ndarray
        Solved potential distribution on the grid.
    """

    eta = eta_grid[:, 0]
    nu = nu_grid[0]
    h_eta = float(eta[1] - eta[0]) if len(eta) > 1 else 1.0
    h_nu = float(nu[1] - nu[0]) if len(nu) > 1 else 1.0

    phi = np.zeros_like(eta_grid)
    ...
    if charge_density is None:
        charge_density = np.zeros_like(phi)

    f = 1.0  # Use unit focal length here; scale absorbed in coordinates
    ...
    for _ in range(max_iter):
        ...
        for i in range(1, phi.shape[0] - 1):
            for j in range(1, phi.shape[1] - 1):
                ...
                a_e = ((eta_i + 0.5 * h_eta) ** 2 - 1.0) / h_eta**2
                a_w = ((eta_i - 0.5 * h_eta) ** 2 - 1.0) / h_eta**2
                a_n = (1.0 - (nu_j + 0.5 * h_nu) ** 2) / h_nu**2
                a_s = (1.0 - (nu_j - 0.5 * h_nu) ** 2) / h_nu**2
                a_p = a_e + a_w + a_n + a_s

                rhs = -charge_density[i, j] * f**2 * (eta_i**2 - nu_j**2)
                ...
                phi_new = (1 - omega) * phi[i, j] + omega * phi_new

                diff = abs(phi_new - phi[i, j])
                if diff > max_diff:
                    max_diff = diff
